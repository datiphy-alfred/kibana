...
import PluginsKibanaDiscoverHitSortFnProvider from 'plugins/kibana/my_discover/_hit_sort_fn';
...
import indexTemplate from 'plugins/kibana/my_discover/index.html';
import StateProvider from 'ui/state_management/state';

const app = uiModules.get('apps/my_discover', [
  'kibana/notify',
  'kibana/courier',
...
app.directive('myDiscoverApp', function () {
  return {
    restrict: 'E',
    controllerAs: 'myDiscoverApp',
    controller: myDiscoverController
  };
});

function myDiscoverController($scope, config, courier, $route, $window, Notifier,
  ...
  //'Discover'
  const notify = new Notifier({ location: 'MyDiscover' });
  ...
  $scope.topNavMenu = [{
    ...
    key: 'save',
    description: 'Save Search (MyDiscover)',
    template: require('plugins/kibana/my_discover/partials/save_search.html')
  }, {
    key: 'open',
    description: 'Load Saved Search (MyDiscover)',
    template: require('plugins/kibana/my_discover/partials/load_search.html')
  }, {
    key: 'share',
    description: 'Share Search (MyDiscover)',
    template: require('plugins/kibana/my_discover/partials/share_search.html')
  }];
...
/*** BEG: INSERT FOR TIME SAVE ***/
  ...
  if (savedSearch.timeRestore && savedSearch.timeTo && savedSearch.timeFrom) { // && !getAppState.previouslyStored()) {
    ...
    //   timefilter.refreshInterval = savedSearch.refreshInterval;
    // }
  }

/*** END: INSERT FOR TIME SAVE ***/
  ...
  $scope.opts.saveDataSource = function () {
    ...
    .then(function () {
      ...
      savedSearch.timeFrom = savedSearch.timeRestore ? timefilter.time.from : undefined;
      savedSearch.timeTo   = savedSearch.timeRestore ? timefilter.time.to : undefined;
      ...
      .then(function (id) {
        ...
        if (id) {
          ...
          if (savedSearch.id !== $route.current.params.id) {
            kbnUrl.change('/my_discover/{{id}}', { id: savedSearch.id });
          } else {
