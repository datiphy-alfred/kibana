...
import createTemplate     from 'plugins/kibana/my_management/sections/indices/_create.html';
import RefreshKibanaIndex from 'plugins/kibana/my_management/sections/indices/_refresh_kibana_index';

uiRoutes
.when('/my_management/kibana/index/', {
  template: createTemplate
});

uiModules.get('apps/my_management')
.controller('myManagementIndicesCreate', function ($scope, kbnUrl, Private, Notifier, indexPatterns, es, config, Promise) {
      ...
      .then(function (id) {
        if (id) {
          refreshKibanaIndex().then(function () {
            ...
            kbnUrl.change('/my_management/kibana/indices/' + indexPattern.id);
          });
        }
      });
