...
import objectViewHTML from 'plugins/kibana/my_management/sections/objects/_view.html';
import IndexPatternsCastMappingTypeProvider from 'ui/index_patterns/_cast_mapping_type';
import uiRoutes from 'ui/routes';
import uiModules from 'ui/modules';

uiRoutes
.when('/my_management/kibana/objects/:service/:id', {
  template: objectViewHTML
});

uiModules.get('apps/my_management')
.directive('myKbnManagementObjectsView', function (kbnIndex, Notifier) {
  return {
    restrict: 'E',
    controller: function ($scope, $injector, $routeParams, $location, $window, $rootScope, es, Private) {
      ...
      function redirectHandler(action) {
        ...
        .then(function (resp) {
          ...
          $location.path('/my_management/kibana/objects').search({
            _a: rison.encode({
