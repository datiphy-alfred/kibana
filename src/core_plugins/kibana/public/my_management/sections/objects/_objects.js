...
import registry from 'plugins/kibana/my_management/saved_object_registry';
import objectIndexHTML from 'plugins/kibana/my_management/sections/objects/_objects.html';
...
uiRoutes
.when('/my_management/kibana/objects', {
  template: objectIndexHTML
});

uiModules.get('apps/my_management')
.directive('myKbnManagementObjects', function (kbnIndex, Notifier, Private, kbnUrl, Promise) {
  return {
    restrict: 'E',
    controller: function ($scope, $injector, $q, AppState, es) {
      ...
      $scope.edit = function (service, item) {
        ...
        kbnUrl.change('/my_management/kibana/objects/{{ service }}/{{ id }}', params);
      };
